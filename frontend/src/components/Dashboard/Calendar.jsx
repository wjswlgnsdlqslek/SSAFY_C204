    ...
    <div className="bg-white relative rounded-lg shadow-lg text-black h-full px-4 overflow-scroll scroll container mx-auto p-4">
      <div className="flex bg-white z-10">
        <div className="relative">
          <button
            className="btn bg-gray-100 p-2 rounded-md shadow-md"
            onClick={() => setShowFilters(!showFilters)}
          >
            필터 옵션
          </button>
          {showFilters && (
            <div className="absolute mt-1 bg-white border border-gray-300 rounded-md shadow-lg flex flex-col items-start">
              <Filters
                filter={eventsTypeFilter}
                setFilter={setEventsTypeFilter}
              />
            </div>
          )}
        </div>
